...
\subsection{Formulazione alternativa: \code{join}}
Nella teoria delle categorie, una monade può essere definita in modo alternativo utilizzando la funzione \code{join}, il cui scopo è il \textit{flattening} di due livelli di computazione nidificati:
\begin{haskellbox}{}
join :: Monad m => m (m a) -> m a
\end{haskellbox}

La funzione \code{join} è completamente \textbf{interdefinibile} con l'operatore di bind (\code{>>=}) secondo le seguenti relazioni:
\begin{haskellbox}{}
join x   = x >>= id
p >>= f  = join ((fmap f) p)
\end{haskellbox}
L'operazione di bind può quindi essere vista come la combinazione sequenziale di una mappatura funzionale (\code{fmap}) seguita da un appiattimento strutturale (\code{join}).

\section{TODO: Generic Programming}
